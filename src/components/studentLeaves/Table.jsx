...
const StudentLeaveTable = () => {
  ...
  const fetchLeaves = async () => {
    try {
      const response = await axios.get(
        "https://ems-backend-hazel.vercel.app/api/student-leave",
        {
          headers: {
      ...
      console.error("An error occurred fetching student leaves:", error);
      if (error.response && !error.response.data.success) {
        alert(error.response.data.error || "Failed to load requests");
      } else {
  ...
  const filterByInput = (e) => {
    ...
    const data = leaves.filter(
      (leave) =>
        leave.matricule.toLowerCase().includes(query) ||
          ...
          {filteredLeaves.length === 0 && (
            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200 mt-4">
              <p className="text-gray-400">
                No student absence requests found.
              </p>
            </div>
          )}
        </div>
